...
class roboServer {
    ...
    handleInvite(req, res, roboPirate) {
        if (req.body.name && req.body.email && req.body.type){
            ...
            res.end("<meta charset='UTF-8'>Du bliver inviteret hurtigst muligt. Vent " +
    ...
    handleAnswer(req, res, userToken, channels='') {
        ...
        const email = payload.original_message.text.match(/\'<mailto:([a-zA-Z]*@.*)\|/)[1]
        console.log(payload);
        ...
        if(payload.actions[0].value === "yes-barn") {
            ...
            fetch(`https://slack.com/api/users.admin.invite?token=${userToken}&channels=${channels}&email=${email}&restricted=true`, {
                method: 'GET',
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                }
            }).then(x => x.json()).then(console.log).catch(console.log)
     //           `https://slack.com/api/users.admin.invite`, token=${userToken}&email=${payload.original_message.text.match(/\'([a-zA-Z]*@.*)\'/)[1]}&channels=${channels}&restricted=true`)
            ...
            fetch(`https://slack.com/api/users.admin.invite?token=${userToken}&channels=${channels}&email=${email}`, {
                //method: 'POST',
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                }
    ...
    async run() {
        ...
        this.app.post('/command', (req, res) =>
            this.handleAnswer(req, res, this.userToken, this.joinChannels));
        // Starts the server
